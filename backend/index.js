...
document.querySelector(".print_terms_button").addEventListener("click", () => {
  ...
  doc.addImage(`${signaturePadData}`, "image/png", 46, 65, 10, 10);
  ...
  doc.text(
    "be less than the minimum amount of the quotation provided by Samsung and/or its authorized service",
    10,
    85
  );
  doc.text("center.", 10, 90);
  ...
  doc.text("authorized separately in writing by Samsung.", 10, 105);
  doc.text(
    "Any handwritten notes made on this service order or job card that provide for an exchange of the device that is",
    10,
    110
  );
  doc.text(
    "granted verbally by Samsung or its authorized service center are not permitted, and an exchange of your device",
    10,
    115
  );
  doc.text("will not be provided in such instances.", 10, 120);
  ...
  doc.text("manufacturer warranty terms and conditions.", 10, 150);
  doc.addImage(`${signaturePadData}`, "image/png", 120, 145, 10, 10);
  ...
  doc.text(
    "specified within the owner's manual warranty terms and conditions. Repair, however, can be offered at a cost",
    10,
    165
  );
  doc.text("to you, the end-user.", 10, 170);
  ...
  doc.addImage(`${signaturePadData}`, "image/png", 80, 205, 10, 10);

  // // canvas integration
  // doc.rect(120, 216, 80, 50, "S");
  // doc.text("Signature", 152, 208);
  // // Add canvas signature
  // doc.addImage(`${signaturePadData}`, "image/png", 128, 220, 80, 50);
