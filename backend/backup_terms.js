...
const termBoldOneCheckbox = document.getElementById("backup_term_bold_1");
const termBoldTwoCheckbox = document.getElementById("backup_term_bold_2");
...
const fetchTerms = async () => {
  const terms = document.querySelector(".tab_1");
  const response = await fetch(
    `http://192.168.1.126:8006/backup_terms_and_conditions`
  );
  const data = await response.json();
  const filteredBoldNotTrue = data.filter((term) => term.is_bold !== true);
...
document
  ...
  .addEventListener("click", (event) => {
    ...
    if (document.getElementById("backup_term_bold_1")?.checked === true) {
      doc.addImage(`${signaturePadData}`, "image/png", 78, 80, 10, 10);
    } else if (
      document.getElementById("backup_term_bold_1")?.checked !== true
    ) {
      doc.setFontSize(8);
      doc.text("(Customer did not acknowledge)", 80, 85);
    }
    // doc.addImage(`${signaturePadData}`, "image/png", 78, 80, 10, 10);
    ...
    if (document.getElementById("backup_term_bold_2")?.checked === true) {
      doc.addImage(`${signaturePadData}`, "image/png", 106, 100, 10, 10);
    } else if (
      document.getElementById("backup_term_bold_2")?.checked !== true
    ) {
      doc.setFontSize(8);
      doc.text("(Customer did not acknowledge)", 107, 105);
    }
    // doc.addImage(`${signaturePadData}`, "image/png", 106, 100, 10, 10);
    ...
    doc.setFontSize(10);
    doc.text(
      "the repair process and/or backup and restore procedures.",
      10,
      155
    );
